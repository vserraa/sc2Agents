...
class ConstructorAgent():
    ...
    async def offensive_force_buildings(self):
        if self.game.units(PYLON).ready.exists:
            ...
            elif len(self.game.units(GATEWAY)) + self.game.already_pending(GATEWAY) < ((self.game.iteration / self.game.ITERATIONS_PER_MINUTE)/2) and len(self.game.units(GATEWAY)) + self.game.already_pending(GATEWAY) < self.MAX_GATEWAY:
            ...
            if self.game.units(CYBERNETICSCORE).ready.exists:
                if len(self.game.units(STARGATE)) + self.game.already_pending(STARGATE) < ((self.game.iteration / self.game.ITERATIONS_PER_MINUTE)/2) and len(self.game.units(STARGATE)) + self.game.already_pending(STARGATE) < self.MAX_STARGATE:
